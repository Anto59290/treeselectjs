...
type DiacriticsMap = { [key: string]: string };

const diacriticsMap : DiacriticsMap = {
  '\u24B6': 'A',
  '\uFF21': 'A',
  '\u00C0': 'A',
  '\u00C1': 'A',
...
export const updateVisibleBySearchTreeItemOptions = (optionsTreeMap: OptionsTreeMap, searchText: string) => {
  optionsTreeMap.forEach((option) => {
    const isSearched = removeDiacritics(option.name).toLowerCase().includes(removeDiacritics(searchText).toLowerCase())
